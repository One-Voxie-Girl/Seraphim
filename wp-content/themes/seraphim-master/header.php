...
o.onload=function(){window.trackingFunctions.onLoad({appId:"692483b38090f500154bbe1e"})},
...
<header class="site-header fixed-top">
  <nav class="navbar navbar-expand-lg w-100 d-flex justify-content-between align-items-center px-4">
...
wp_nav_menu([
  ...
  'walker' => new class extends Walker_Nav_Menu {
    ...
    // Track which top-level item should get repeater appended
    protected $appendMegaForId = null;

    function start_lvl( &$output, $depth = 0, $args = null ) {
      $indent = str_repeat("\t", $depth);
      // Keep your existing submenu markup
      $output .= "\n$indent<ul class=\"sub-menu dropdown-menu mini_menu\">\n";
    }

    function end_lvl( &$output, $depth = 0, $args = null ) {
      $indent = str_repeat("\t", $depth);

      // Append repeater content INSIDE the same dropdown for top-level items
      if ($depth === 0 && $this->appendMegaForId) {

        // Build your repeater HTML and append to $output
        ob_start();
        if (have_rows('mega_menu_content', $this->appendMegaForId)) {
          echo '<li class="featuredMenuArea"><div class="row">';
          while (have_rows('mega_menu_content', $this->appendMegaForId)) {
            the_row();
            $post_object = get_sub_field('featured_content');
            if ($post_object) {
              $post = $post_object;
              setup_postdata($post);

              echo '<div class="col-12 col-lg-4 mega-cta">';
              echo '<a href="' . esc_url(get_permalink($post)) . '">';
              if (has_post_thumbnail($post)) {
                echo '<div class="featuredImg">' . get_the_post_thumbnail($post, 'medium') . '</div>';
              }
              echo '<h4>' . esc_html(get_the_title($post)) . '</h4>';
              echo '<div class="call_to_action peach"><span class="cta-line">Read more</span></span></div>';
              echo '</a>';
              echo '</div>';
            }
          }
          wp_reset_postdata();
          echo '</div></li>'; // .featuredMenuArea
        }
        $output .= ob_get_clean();

        // Clear the flag so it doesn’t bleed into the next item
        $this->appendMegaForId = null;
      }
    ...
    function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
      ...
      // Bootstrap dropdown classes for parents
      ...
      // If this is a top-level parent and it HAS repeater rows, set the flag.
      // We'll append the repeater inside the same dropdown in end_lvl().
      if ($depth === 0 && in_array('menu-item-has-children', $classes) && have_rows('mega_menu_content', $item->ID)) {
        $this->appendMegaForId = (int) $item->ID;
      }
    }
    ...
    function end_el( &$output, $item, $depth = 0, $args = null ) {
      $output .= "</li>\n";
      if ($depth === 0) {
        // Safety: ensure flag is cleared after finishing a top-level item
        $this->appendMegaForId = null;
      }
    }
  }
...
?>
    ...
    <div class="d-none d-lg-block">
      <div class="call_to_action text-bold">
        <a href="/contact" rel="bookmark" class="cta-line">
    ...
    <!-- Mobile toggle -->
    <button class="navbar-toggler collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#primaryNav" aria-controls="primaryNav" aria-expanded="false" aria-label="<?php esc_attr_e('Toggle navigation', 'make-us-care-3-0'); ?>">
      <div class="navbar-toggler-line"></div>
      <div class="navbar-toggler-line second"></div>
    </button>
